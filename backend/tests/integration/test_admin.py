...
@pytest.mark.asyncio
async def test_ingest_podcast_routes_to_podcast_service(
    client: AsyncClient,
) -> None:
    podcast_result = IngestResult(
        source_type="podcast",
        language="fr",
        query="actualites",
        total_fetched=3,
    ...
    with (
        ...
        patch(
            ...
            new=AsyncMock(return_value=podcast_result),
        ),
    ):
...
@pytest.mark.asyncio
async def test_ingest_article_routes_to_article_service(
    client: AsyncClient,
) -> None:
    ...
    with (
        patch("app.api.routes.admin.settings") as s,
        patch(
            "app.api.routes.admin.ingest_service.ingest_article",
            new=AsyncMock(return_value=article_result),
        ),
    ):
        s.ADMIN_API_KEY = "test-key"
        r = await client.post(
    ...
    assert r.status_code == 200
    assert r.json()["source_type"] == "article"
